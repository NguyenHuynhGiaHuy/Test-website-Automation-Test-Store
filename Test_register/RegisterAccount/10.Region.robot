...
*** Test Cases ***
Register with the Region / State: having no characters (empty) and check the constraint: [Documentation] Đăng ký với trường Region / State: không có ký tự nào (rỗng) và kiểm tra ràng buộc:
...
Register with the login field with the correct Region / State field after selecting Country and checking constraints

    [Documentation]    Đăng kí với trường đăng nhập với trường Region / State chính xác sau khi chọn Country  và kiểm tra ràng buộc
    ...
    Sleep    10seconds
    Select From List by Label    id=AccountFrm_zone_id    Ha Noi
