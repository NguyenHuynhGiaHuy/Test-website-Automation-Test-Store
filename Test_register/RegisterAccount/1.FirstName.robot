...
*** Test Cases ***
Register with the First Name field having no characters (empty) and check the constraint:
    [Documentation]    Đăng kí với trường First Name không có ký tự nào (rỗng) và kiểm tra ràng buộc:
    ...
    Fail    Test Case Failed due to validation error.    Log    Input vào trường FistName rỗng
...
Register with the First Name field reaching the minimum and maximum length:    
...    [Documentation]    Đăng kí với trường First Name đạt độ dài tối thiểu và tối đa:
    ...
    # Bước 4: Đóng trình duyệt
    Close Browser
Register with a First Name field longer than 32 characters:    [Documentation]    Đăng kí với trường First Name dài hơn 32 ký tự:
    Open Browser    ${URL}    chrome
    Maximize Browser Window
    Set Selenium Speed    0.5
    ...
    # Bước 4: Đóng trình duyệt
    Close Browser
Register with the First Name field containing only special characters:    [Documentation]    Đăng kí với trường First Name chỉ chứa ký tự đặc biệt:
    Open Browser    ${URL}    chrome
    Maximize Browser Window
    Set Selenium Speed    0.5
    ...
    # Bước 4: Đóng trình duyệt
    Close Browser
Register with the First Name field exactly 1 character long:    [Documentation]    Đăng kí với trường First Name có độ dài chính xác 1 ký tự:
    Open Browser    ${URL}    chrome
    Maximize Browser Window
    Set Selenium Speed    0.5
...
Register with the First Name field exactly 32 characters long:    [Documentation]    Đăng kí với trường First Name có độ dài chính xác 32 ký tự:
    Open Browser    ${URL}    chrome
    Maximize Browser Window
    Set Selenium Speed    0.5
...
Register with the First Name field containing alphanumeric characters, full length:    [Documentation]    Đăng kí với trường First Name chứa ký tự chữ cái và số, đủ độ dài:
    Open Browser    ${URL}    chrome
    Maximize Browser Window
    Set Selenium Speed    0.5
