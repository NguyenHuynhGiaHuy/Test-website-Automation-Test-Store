...
*** Test Cases ***
Register with the Country having no characters (empty) and check the constraint: [Documentation] Đăng ký với trường Country không có ký tự nào (rỗng) và kiểm tra ràng buộc:
...
Register with the login field with the correct Country (v): and check the constraint

    [Documentation]   Đăng kí với trường đăng nhập với Country chính xác (v):và kiểm tra ràng buộc
