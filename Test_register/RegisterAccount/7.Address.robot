...
*** Test Cases ***
Register with the Address having no characters (empty) and check the constraint: [Documentation] Đăng ký với trường Address không có ký tự nào (rỗng) và kiểm tra ràng buộc:
...
Register with the Address field reaching min and max length: and check the constraint
    [Documentation]    Đăng kí với trường Address đạt độ dài tối thiểu và tối đa:và kiểm tra ràng buộc
...
Register with the Address field containing special characters: and check the constraint
    [Documentation]    Đăng kí với trường Address chứa ký tự đặc biệt:và kiểm tra ràng buộc
    Open Browser    ${URL}    chrome
    Maximize Browser Window
    Set Selenium Speed    0.5
    ...
    Input Text    id=AccountFrm_address_1    123 ABC Street, #456, XYZ Ward
...
Register with the Address field containing only space characters: and check the constraint
    [Documentation]    Đăng kí với trường Address chỉ chứa ký tự khoảng trắng:và kiểm tra ràng buộc
    Open Browser    ${URL}    chrome
    Maximize Browser Window
    Set Selenium Speed    0.5
    ...
    Input Text    id=AccountFrm_address_1    1 2 3 Đ ư ờ n g L ê L a i, Q u ậ n  1, T h àn h p h ố Hồ Chí Minh
...
Register with an Address field greater than 255 characters in length (limited length for an address in some systems): and check the constraint
    [Documentation]   Đăng kí với trường Address có độ dài lớn hơn 255 ký tự (độ dài giới hạn cho một địa chỉ trong một số hệ thống):và kiểm tra ràng buộc
...
Register with the Address field being exactly 255 characters long (limited length for an address in some systems): and check the constraint
    [Documentation]   Đăng kí với trường Address có độ dài chính xác 255 ký tự (độ dài giới hạn cho một địa chỉ trong một số hệ thống): và kiểm tra ràng buộc
...
Register with the Address field containing special characters <3 characters: and check the constraint
    [Documentation]   Đăng kí với trường  Address chứa kí tự đặc biệt <3 kí tự:và kiểm tra ràng buộc
    Open Browser    ${URL}    chrome
    Maximize Browser Window
    Set Selenium Speed    0.5
    ...
    Input Text    id=AccountFrm_address_1    ~
