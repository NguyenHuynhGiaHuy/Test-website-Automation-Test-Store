...
*** Test Cases ***
Register with the Password Confirm: having no characters (empty) and check the constraint:
    [Documentation] Đăng ký với trường Password Confirm: không có ký tự nào (rỗng) và kiểm tra ràng buộc:
...
Register with the Password Confirm: field of minimum length and containing only alphanumeric characters: and check the constraint
    [Documentation]    Đăng kí với trường Password Confirm: đạt độ dài tối thiểu và chỉ chứa ký tự chữ cái và số: và kiểm tra ràng buộc
...
Register with a Password Confirm: field longer than 20 characters: and check the constraints
    [Documentation]   Đăng kí với trường Password Confirm: dài hơn 20 ký tự:và kiểm tra ràng buộc
    Open Browser    ${URL}    chrome
    Maximize Browser Window
    Set Selenium Speed    0.5
    ...
    Input Text    id=AccountFrm_confirm    thisisalongpasswordthatisdefinitelymorethan20characterslong
...
Register with the Password Confirm: field shorter than 4 characters: and check the constraints
    [Documentation]   Đăng kí với trường  Password Confirm: ngắn hơn 4 ký tự: và kiểm tra ràng buộc
    Open Browser    ${URL}    chrome
    Maximize Browser Window
    Set Selenium Speed    0.5
   ...
   #first name
    ...
    Input Text    id=AccountFrm_confirm    abc
...
Register with the Password Confirm: field containing special characters: and check the constraints

    [Documentation]   Đăng kí với trường Password Confirm: chứa ký tự đặc biệt:và kiểm tra ràng buộc
   ...
   #first name
    ...
    Input Text    id=AccountFrm_confirm    pass!@#123
...
Register with the Password Confirm: field containing only special characters and check the constraints
    [Documentation]   Đăng kí với trường Password Confirm: chỉ chứa ký tự đặc biệt và kiểm tra ràng buộc
   ...
   #first name
    ...
    Input Text    id=AccountFrm_confirm    $%^&*_*&%
...
Register with the Password Confirm: field less than 5 characters in length: and check the constraint
    [Documentation]   Đăng kí với trường  Password Confirm: có độ dài nhỏ hơn 5 ký tự: và kiểm tra ràng buộc
   ...
   #first name
    ...
    Input Text    id=AccountFrm_confirm    ha ha ha
...
Register with a Password Confirm: field that has a combination of letters, numbers and special characters, of all length: and check the constraints
    [Documentation]   Đăng kí với trường  Password Confirm: có kết hợp ký tự chữ cái, số và ký tự đặc biệt, đủ độ dài:và kiểm tra ràng buộc
   ...
   #first name
    ...
    Input Text    id=AccountFrm_confirm    P@ssw0rd!
...
Register with the Password Confirm: field exactly 4 characters long: and check the constraints

    [Documentation]  Đăng kí với trường Password Confirm: có độ dài chính xác 4 ký tự:và kiểm tra ràng buộc
   ...
   #first name
    ...
    Input Text    id=AccountFrm_confirm    pass
