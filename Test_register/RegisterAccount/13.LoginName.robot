...
*** Test Cases ***
Register with the LoginName having no characters (empty) and check the constraint: [Documentation] Đăng ký với trường LoginName không có ký tự nào (rỗng) và kiểm tra ràng buộc:
...
Register with the Login name field of minimum length and containing only alphanumeric characters: and check the constraint

    [Documentation]   Đăng kí với trường Login name đạt độ dài tối thiểu và chỉ chứa ký tự chữ cái và số: và kiểm tra ràng buộc
...
Register with the Login name field longer than 64 characters: and check the constraint

    [Documentation]    Đăng kí với trường Login name dài hơn 64 ký tự: và kiểm tra ràng buộc
    Open Browser    ${URL}    chrome
    Maximize Browser Window
    Set Selenium Speed    0.5
    ...
    Input Text    id=AccountFrm_loginname   thisisalongloginnamethatisdefinitelymorethan64characterslongggggggggggggggggggg
...
Register with the Login name field containing the special character: and check the constraint

    [Documentation]    Đăng kí với trường Login name chứa ký tự đặc biệt: và kiểm tra ràng buộc
    ...
    Input Text    id=AccountFrm_loginname   user@123
...
Register with the Login name field containing the space : and check the constraint

    [Documentation]    Đăng kí với trường Login name chứa khoảng trắng :và kiểm tra ràng buộc
    ...
    Input Text    id=AccountFrm_loginname   user name
...
Register with the Login name field containing special characters and full length: and check the constraints


    [Documentation]    Đăng kí với trường Login name chứa ký tự đặc biệt và đủ độ dài: và kiểm tra ràng buộc
    ...
    Input Text    id=AccountFrm_loginname   test!@#123
...
Register with the Login name field less than 5 characters in length: and check the constraint
    ...
    Input Text    id=AccountFrm_loginname   a
...
Register with the Login name field containing no alphabetic or numeric characters: and check the constraint


    [Documentation]    Đăng kí với trường Login name không chứa ký tự chữ cái hoặc số: và kiểm tra ràng buộc
    ...
    Input Text    id=AccountFrm_loginname   $%^&&*
...
Register with the Login name field properly qualified: and check the constraint


    [Documentation]    Đăng kí với trường Login name đủ điều kiện đúng: và kiểm tra ràng buộc
    Open Browser    ${URL}    chrome
    Maximize Browser Window
    Set Selenium Speed    0.5
    ...
    Input Text    id=AccountFrm_loginname   user123
