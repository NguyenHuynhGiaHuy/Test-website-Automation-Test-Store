...
*** Test Cases ***
Register with the Last Name field having no characters (empty) and check the constraint:    [Documentation]    Đăng ký với trường Last Name không có ký tự nào (rỗng) và kiểm tra ràng buộc:
...
Register with the Last Name field reaching the minimum and maximum length:
    [Documentation]    Đăng kí với trường Last Name đạt độ dài tối thiểu và tối đa:
    ...
    Input Text    id=AccountFrm_lastname    Doe
...
DangKiTaiKhoanMoi-1.1
    [Documentation]    Last Name dài hơn 32 ký tự:
    Open Browser    ${URL}    chrome
    Maximize Browser Window
    Set Selenium Speed    0.5
    ...
    Input Text    id=AccountFrm_lastname    ThisIsAVeryLongLastNameThatIsDefinitelyMoreThan32CharactersLong
    # Bước 3: Xác nhận và đăng kí
    Click Button    xpath=//*[@id="AccountFrm"]/div[5]/div/div/button
    Sleep    10s
...
DangKiTaiKhoanMoi-1.2
    [Documentation]    Last Name chỉ chứa ký tự đặc biệt:
    ...
    Input Text    id=AccountFrm_lastname    !@#$%^&*
...
DangKiTaiKhoanMoi-1.3
    [Documentation]    Last Name có độ dài chính xác 1 ký tự:
    Open Browser    ${URL}    chrome
    Maximize Browser Window
    Set Selenium Speed    0.5
    ...
    Input Text    id=AccountFrm_lastname    D
...
DangKiTaiKhoanMoi-1.4
    ...
    Input Text    id=AccountFrm_lastname    DoeDoeDoeDoeDoeDoeDoeDoeDoeDoeDoeDoeDoeDoeDoeDoe
    # Bước 3: Xác nhận và đăng kí
    Click Button    xpath=//*[@id="AccountFrm"]/div[5]/div/div/button
    Sleep    10s
...
DangKiTaiKhoanMoi-1.5
    [Documentation]    Last Name chứa ký tự chữ cái và số, đủ độ dài:
    Open Browser    ${URL}    chrome
    Maximize Browser Window
    Set Selenium Speed    0.5
